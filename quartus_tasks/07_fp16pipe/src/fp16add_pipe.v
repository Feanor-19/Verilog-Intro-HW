...
reg [BEXP_BITS-1:0] bexp_a, bexp_a_q;
reg                 sign_a;
...
reg [MANTT_BITS-1:0] mantt1, mantt1_q;
reg [MANTT_BITS-1:0] mantt2;
reg [BEXPT_BITS-1:0] bexpt;
reg [MANTT_POS_BITS-1:0] lead1_pos, lead1_pos_q;
reg                  lead1_pos_vld, lead1_pos_vld_q;

reg                 sign_res1, sign_res1_q;
...
reg                 pass_res1, pass_res1_q;
reg [FP16_BITS-1:0] res1, res1_q;
...
always @(*) begin
    ...
    res1 = {sign_res1, bexp_res1, mant_res1};
end
...
always @(posedge clk) begin
    lead1_pos_q     <= lead1_pos;
    lead1_pos_vld_q <= lead1_pos_vld;
    mantt1_q        <= mantt1;
    sign_res1_q     <= sign_res1;
    bexp_a_q        <= bexp_a;
    pass_res1_q     <= pass_res1;
    res1_q          <= res1;
end
...
always @(*) begin
    ...
    mantt2 = mantt1_q;
    sign_res2 = sign_res1_q;

    if (lead1_pos_vld_q) begin
        if (lead1_pos_q == MANTT_C_BIT) begin
            ...
            mantt2 = mantt2 >> 1'b1;
            bexpt = BEXPT_BITS'(bexp_a_q) + 1;
        end else if (lead1_pos_q < MANTT_BITS-2) begin
            mantt2 = mantt2 << ((MANTT_BITS-2) - lead1_pos_q);
            bexpt = BEXPT_BITS'(bexp_a_q) - ((MANTT_BITS-2) - BEXPT_BITS'(lead1_pos_q));
        end else begin
            // mantt2 isn't changed
            bexpt = {{(BEXPT_BITS-BEXP_BITS){1'b0}}, bexp_a_q};
        end
